...
eval (EditLocation editLocation) state = do
  if state.data.driverInfoCardState.distance < 100 then do
    void $ pure $ toast $ getString DRIVER_ALMOST_AT_PICKUP
    continue state
  else if (state.data.driverInfoCardState.editPickupAttemptsLeft <= 0) then do
    void $ pure $ toast $ "You have reached the maximum number of attempts to change the pickup location"
    continue state
  else do 
...
eval (UpdateLocation key lat lon) state = do
  ...
  case key of
    "LatLon" -> do
      exit $ UpdateLocationName state{props{defaultPickUpPoint = "", markerLabel = "", rideSearchProps{ sourceManuallyMoved = sourceManuallyMoved, destManuallyMoved = destManuallyMoved }}} latitude longitude
    _ ->  if length (filter( \ (item) -> (item.place == key)) state.data.nearByPickUpPoints) > 0 then do
            exit $ UpdateLocationName state{props{defaultPickUpPoint = key , markerLabel = "", rideSearchProps{ sourceManuallyMoved = sourceManuallyMoved, destManuallyMoved = destManuallyMoved }}} latitude longitude
          else continue state
...
eval (UpdatePickupLocation  key lat lon) state = do
  ...
  case (STR.replace (STR.Pattern "LocationIsFar") (STR.Replacement "") key) of
    "LatLon" -> do
      exit $ UpdatePickupName state{props{defaultPickUpPoint = "", markerLabel = if STR.contains (STR.Pattern "LocationIsFar") key then "Location is too far!" else "", rideSearchProps{ sourceManuallyMoved = sourceManuallyMoved}}} latitude longitude
    _ -> do
      ...
      case focusedIndex of
        Just index -> do
          ...
          exit $ UpdatePickupName state{props{defaultPickUpPoint = key, markerLabel = if STR.contains (STR.Pattern "LocationIsFar") key then "Location is too far!" else "", rideSearchProps{ sourceManuallyMoved = sourceManuallyMoved}}} latitude longitude
        Nothing -> continue state
