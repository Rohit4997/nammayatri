...
sendIssue (personId, merchantId) request = do
  ...
  ticketRequest <- mkTicket newIssue person phoneNumber merchant.kaptureDisposition
...
safetyCheckSupport (personId, merchantId) req = do
  ...
  ticketRequest <- ticketReq ride person phoneNumber req.description (merchant.kaptureDisposition)
  when (not req.isSafe && riderConfig.enableSupportForSafety) $ void $ createTicket person.merchantId moCityId $ ticketRequest
  return Success
  where
    ticketReq ride person phoneNumber description disposition = do
...
mkTicket :: (CacheFlow m r, EsqDBFlow m r, MonadFlow m) => DIssue.Issue -> Person.Person -> Maybe Text -> Text -> m Ticket.CreateTicketReq
mkTicket issue person phoneNumber disposition = do
