...
import qualified Domain.Types.Estimate as DTE
...
makeSearchRequestForDriverAPIEntity :: SearchRequestForDriver -> DSR.SearchRequest -> DST.SearchTry -> Maybe DSM.BapMetadata -> Seconds -> Maybe Money -> Seconds -> Variant.Variant -> Bool -> Bool -> Maybe DTE.Estimate -> SearchRequestForDriverAPIEntity
makeSearchRequestForDriverAPIEntity nearbyReq searchRequest searchTry bapMetadata delayDuration mbDriverDefaultExtraForSpecialLocation keepHiddenForSeconds requestedVehicleVariant isTranslated isValueAddNP mbEstimate =
  SearchRequestForDriverAPIEntity
    { searchRequestId = nearbyReq.searchTryId,
      ...
      driverPickUpCharges = extractDriverPickupChanges,
      specialZoneExtraTip = min nearbyReq.driverMaxExtraFee mbDriverDefaultExtraForSpecialLocation,
      ..
    }
  where
    extractDriverPickupChanges =
      mbEstimate >>= \estimate ->
        estimate.fareParams >>= \fP -> case fP.fareParametersDetails of
          DFParams.ProgressiveDetails det -> Just det.deadKmFare
          _ -> Nothing
