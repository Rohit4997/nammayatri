...
findAllByMerchantId :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Kernel.Prelude.Maybe (Kernel.Types.Id.Id Domain.Types.Merchant.Merchant) -> m ([Domain.Types.SuspectStatusChangeRequest.SuspectStatusChangeRequest])
findAllByMerchantId merchantId = do
  findAllWithKV
    [ Se.Is Beam.merchantId $ Se.Eq (Kernel.Types.Id.getId <$> merchantId)
    ]

findAllByReqStatus :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Domain.Types.SuspectFlagRequest.AdminApproval -> m ([Domain.Types.SuspectStatusChangeRequest.SuspectStatusChangeRequest])
findAllByReqStatus reqStatus = do
  findAllWithKV
    [ Se.Is Beam.reqStatus $ Se.Eq reqStatus
    ]

findBySuspectId :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Kernel.Prelude.Text -> m (Maybe (Domain.Types.SuspectStatusChangeRequest.SuspectStatusChangeRequest))
findBySuspectId suspectId = do
  findOneWithKV
    [ Se.Is Beam.suspectId $ Se.Eq suspectId
    ]

findBySuspectIdAndMerchantId :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Kernel.Prelude.Text -> Kernel.Prelude.Maybe (Kernel.Types.Id.Id Domain.Types.Merchant.Merchant) -> m (Maybe (Domain.Types.SuspectStatusChangeRequest.SuspectStatusChangeRequest))
findBySuspectIdAndMerchantId suspectId merchantId = do
  findOneWithKV
    [ Se.And
        [ Se.Is Beam.suspectId $ Se.Eq suspectId,
          Se.Is Beam.merchantId $ Se.Eq (Kernel.Types.Id.getId <$> merchantId)
...
findByPrimaryKey :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Kernel.Types.Id.Id Domain.Types.SuspectStatusChangeRequest.SuspectStatusChangeRequest -> m (Maybe (Domain.Types.SuspectStatusChangeRequest.SuspectStatusChangeRequest))
...
updateByPrimaryKey :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Domain.Types.SuspectStatusChangeRequest.SuspectStatusChangeRequest -> m ()
