...
findByConfigName :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Kernel.Prelude.Text -> m (Maybe (Domain.Types.PortalConfigs.PortalConfigs))
findByConfigName configName = do
  findOneWithKV
    [ Se.Is Beam.configName $ Se.Eq configName
    ]

findByPrimaryKey :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Kernel.Types.Id.Id Domain.Types.PortalConfigs.PortalConfigs -> m (Maybe (Domain.Types.PortalConfigs.PortalConfigs))
...
updateByPrimaryKey :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Domain.Types.PortalConfigs.PortalConfigs -> m ()
updateByPrimaryKey Domain.Types.PortalConfigs.PortalConfigs {..} = do
  _now <- getCurrentTime
  updateWithKV
    [ Se.Set Beam.configName configName,
