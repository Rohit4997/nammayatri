...
findByMerchantIdAndreadStatus :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Domain.Types.Notification.NotificationCategory -> Kernel.Prelude.Bool -> Kernel.Prelude.Maybe (Kernel.Types.Id.Id Domain.Types.Merchant.Merchant) -> m (Maybe (Domain.Types.Notification.Notification))
...
findByReceiverId :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Maybe Int -> Maybe Int -> Kernel.Prelude.Text -> m ([Domain.Types.Notification.Notification])
findByReceiverId limit offset receiverId = do
...
findByReceiverIdAndId :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Kernel.Prelude.Text -> Kernel.Types.Id.Id Domain.Types.Notification.Notification -> m (Maybe (Domain.Types.Notification.Notification))
findByReceiverIdAndId receiverId (Kernel.Types.Id.Id id) = do
  findOneWithKV
    [ Se.And
        [ Se.Is Beam.receiverId $ Se.Eq receiverId,
...
updateReadStatusById :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Kernel.Prelude.Bool -> Kernel.Types.Id.Id Domain.Types.Notification.Notification -> m ()
updateReadStatusById readStatus (Kernel.Types.Id.Id id) = do
  _now <- getCurrentTime
  updateWithKV
...
findByPrimaryKey :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Kernel.Types.Id.Id Domain.Types.Notification.Notification -> m (Maybe (Domain.Types.Notification.Notification))
...
updateByPrimaryKey :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Domain.Types.Notification.Notification -> m ()
