...
findByMerchantId :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Kernel.Prelude.Maybe (Kernel.Types.Id.Id Domain.Types.Merchant.Merchant) -> m (Maybe (Domain.Types.MerchantConfigs.MerchantConfigs))
findByMerchantId merchantId = do
  findOneWithKV
    [ Se.Is Beam.merchantId $ Se.Eq (Kernel.Types.Id.getId <$> merchantId)
    ]

findByRequestWebHook :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Kernel.Prelude.Bool -> m ([Domain.Types.MerchantConfigs.MerchantConfigs])
findByRequestWebHook requestWebHook = do
  findAllWithKV
    [ Se.Is Beam.requestWebHook $ Se.Eq requestWebHook
    ]

updateRequestWebHookById :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Kernel.Prelude.Bool -> Kernel.Types.Id.Id Domain.Types.MerchantConfigs.MerchantConfigs -> m ()
...
findByPrimaryKey :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Kernel.Types.Id.Id Domain.Types.MerchantConfigs.MerchantConfigs -> m (Maybe (Domain.Types.MerchantConfigs.MerchantConfigs))
...
updateByPrimaryKey :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Domain.Types.MerchantConfigs.MerchantConfigs -> m ()
